...
{   
  ...
  $query = "SELECT * FROM cyrusbilling.billbook WHERE BranchCode=$BranchCode and Cancelled!=1 and Cancelled!=-1";
  $result = $con2->query($query);

  $sqlGST = "SELECT BranchCode, SUM(TotalBilledValue), Sum(ReceivedAmount) FROM cyrusbilling.billbook WHERE BranchCode=$BranchCode and Cancelled!=1 and Cancelled!=-1 GROUP BY BranchCode";
    ...
    <tbody>
      ...
      if (mysqli_num_rows($result)>0)
      {
       while($row = mysqli_fetch_array($result)){
        ...
        print "<tr>";
        print '<td><a href=billView.php?billno='.$enBillNo.' target="_blank">'.$BillNo.'</a></td>';
