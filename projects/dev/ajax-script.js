 ...
 function myFunction() {
   ...
   console.log(Type);var BranchCode = document.getElementById("BranchC").value;
...
$(document).on('click', '.SaveOrder', function(){
  ...
  var InfoDate = document.getElementById("InfoDateAdd").value;
  var Expected = document.getElementById("ExpectedAdd").value;
  var waranty = document.getElementById("waranty").value;
  ...
  if (InfoDate=='' || Expected=='' || waranty=='' || BGAmount=='' || BGValidity=='' || Description=='') {
    swal("error","Please enter all fields","error");
  }else if(ZoneCode==''){
    swal("error","Please select Zone or Division","error");
  }else{
    $.ajax({
      url:"post.php",
      method:"POST",
      data:{'Zone':ZoneCode, 'InfoDate': InfoDate, 'Expected': Expected, 'Waranty':waranty, 'BGAmount':BGAmount, 'BGValidity':BGValidity, 'Description': Description},
      success:function(data){
        swal("success","Order Registered","success")
      }
    });
  } 
});
