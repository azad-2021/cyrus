...
WHERE month(VisitDate)=month(curdate()) and `cyrus regions`.SubControllerID=$EXEID";
$result = mysqli_query($con3,$sql);
                      ...
                      <table width="100%" class="table display text-start align-middle table-bordered border-primary table-hover mb-0">
                        ...
                        <tbody>
                          ...
                          while($rowE=mysqli_fetch_assoc($resultTech)){
                           ...
                           join cyrusbackend.`cyrus regions` on districts.RegionCode=`cyrus regions`.RegionCode WHERE AssignDate is not null and Attended=0 and datediff(Current_date(), AssignDate)>10 and districts.SubControllerID=$EXEID and EmployeeCode=$EmployeeID";
                           ...
                           join cyrusbackend.`cyrus regions` on districts.RegionCode=`cyrus regions`.RegionCode 
                           WHERE AssignDate is not null and Attended=0 and Discription like '%AMC%' and EmployeeCode=$EmployeeID and districts.SubControllerID=$EXEID and datediff(Current_date(), AssignDate)>60";
                           ...
                           join cyrusbackend.`cyrus regions` on districts.RegionCode=`cyrus regions`.RegionCode 
                           WHERE EmployeeCode=$EmployeeID and AssignDate is not NULL and Attended=0 and Discription not like '%AMC%' and districts.SubControllerID=$EXEID and datediff(Current_date(), AssignDate)>30";
                      ...
                      <table width="100%" class="table display text-start align-middle table-bordered border-primary table-hover mb-0">
                        ...
                        <tbody>
                          ...
                          while($rowE=mysqli_fetch_assoc($resultTech)){
                           ...
                           join `cyrus regions` on districts.RegionCode=`cyrus regions`.RegionCode
                           where AttendDate is not null and Attended=1 and datediff(AttendDate, AssignDate)>10 and districts.SubControllerID=$EXEID and complaints.EmployeeCode=$EmployeeID and month(AttendDate)=month(current_date()) and year(AttendDate)=year(current_date())";
                           $result=mysqli_query($con,$query);
                           $row = mysqli_fetch_array($result);
                           ...
                           join districts on branchdetails.Address3=districts.District
                           join `cyrus regions` on districts.RegionCode=`cyrus regions`.RegionCode
                           where AttendDate is not null and Attended=1 and datediff(AttendDate, AssignDate)>90 and districts.SubControllerID=$EXEID and Discription like '%AMC%'and orders.EmployeeCode=$EmployeeID and month(AttendDate)=month(current_date()) and year(AttendDate)=year(current_date())";
                           $result2=mysqli_query($con,$query2);
                           $row2 = mysqli_fetch_array($result2);
                           $AMC=$row2["count(OrderID)"];
                           ...
                           join districts on branchdetails.Address3=districts.District
                           join `cyrus regions` on districts.RegionCode=`cyrus regions`.RegionCode
                           where AttendDate is not null and Attended=1 and datediff(AttendDate, AssignDate)>30 and districts.SubControllerID=$EXEID and Discription not like '%AMC%'and orders.EmployeeCode=$EmployeeID and month(AttendDate)=month(current_date()) and year(AttendDate)=year(current_date())";
                           $result3 = mysqli_query($con, $query3);
                           $row3 = mysqli_fetch_array($result3);
                           $AO=$row3["count(OrderID)"];
