...
<body>
        ...
        <table class="table table-hover table-sm table-bordered border-primary nowrap">
          <thead>
            <tr>
              ...
              <th style="min-width:150px">Change Srvice Engineer</th>
          ...
          <tbody>
            <tr>
              ...
              <td>
                ...
                <select class="form-control rounded-corner" id="ChangeSrEngineer" id2="<?php echo $JBCRAD; ?>">
                  <option value="">Select</option>
                  <?php 
                  $query="SELECT EmployeeCode, `Employee Name` FROM cyrusbackend.employees WHERE Inservice=1 and EmployeeCode!=85 Order By `Employee Name`";
                  $result=mysqli_query($con,$query);
                  while($rowE=mysqli_fetch_assoc($result)){

                    ?>
                    <option value="<?php echo $rowE["EmployeeCode"] ?>"><?php echo $rowE["Employee Name"] ?></option>
                  <?php } ?>
                </select>
  ...
  <script type="text/javascript">
   ...
   $(document).on('change','#ChangeSrEngineer', function(){
    var EmployeeCode=$(this).val();
    var jobcard = $(this).attr("id2");
    if (confirm("You want to change Employee. Do you wish to continue?")) {
      if (EmployeeCode && jobcard) {
        $.ajax({
         url:"dataget.php",
         method:"POST",
         data:{'Jobcard':jobcard, 'ChangeEmployee':EmployeeCode},
         success:function(data){
          if (data==1) {
