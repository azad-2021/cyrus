...
<script type="text/javascript">
 $(document).on('click','.EditServiceDone', function(){
  var ServiceDone = $(this).attr("id");
  var jobcard = $(this).attr("id2");
  document.getElementById("NewServiceDone").value=ServiceDone; 
  document.getElementById("JobcardServiceDone").value=jobcard;
});

 $(document).on('click','.SaveServiceDone', function(){
  var ServiceDone = document.getElementById("NewServiceDone").value; 
  var jobcard = document.getElementById("JobcardServiceDone").value;
  if (ServiceDone && jobcard) {
    $.ajax({
     url:"dataget.php",
     method:"POST",
     data:{'Jobcard':jobcard, 'ServiceDone':ServiceDone},
     success:function(data){
      swal("success","Updated","success");
      $('#EditServiceDone').modal('hide');
    }
  })
  }
 ...
 $(document).on('click','.EditPendingWork', function(){
  var PendingWork = $(this).attr("id");
  var jobcard = $(this).attr("id2");
  document.getElementById("NewPendingWork").value=PendingWork; 
  document.getElementById("JobcardPendingWork").value=jobcard;
});

 $(document).on('click','.SavePendingWork', function(){
  var PendingWork = document.getElementById("NewPendingWork").value; 
  var jobcard = document.getElementById("JobcardPendingWork").value;
  if (PendingWork && jobcard) {
    $.ajax({
     url:"dataget.php",
     method:"POST",
     data:{'Jobcard':jobcard, 'PendingWork':PendingWork},
     success:function(data){
      swal("success","Updated","success");
      $('#EditPendingWork').modal('hide');
    }
  })
  }
  var delayInMilliseconds = 1000; 
 ...
 $(document).on('change','#ChangeGadgetB', function(){
  var GadgetID=$(this).val();
  var jobcard = $(this).attr("id2");
  if (confirm("You want to change Gadget. Do you wish to continue?")) {
    if (GadgetID && jobcard) {
      $.ajax({
       url:"dataget.php",
       method:"POST",
       data:{'Jobcard':jobcard, 'ChangeGadget':GadgetID},
       success:function(data){
        swal("success","Updated","success");
 ...
 $(document).on('click','.ViewGadgetDetails', function(){
