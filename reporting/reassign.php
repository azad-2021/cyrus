...
if (!empty($Data))
{
	...
	if (!empty($obj->OrderID) and !empty($obj->EmployeeID)) {
		...
		if ($Status=='Unassigned') {
			...
			if ($con->query($sql2) === TRUE) {
				...
				echo "Error: " . $sql2 . "<br>" . $con->error;
				$myfile = fopen("Reassignerr.txt", "w") or die("Unable to open file!");
				fwrite($myfile, $con->error);
				fclose($myfile);
			}
		}
	...
	if (!empty($obj->ComplaintID) and !empty($obj->EmployeeID)) {
		...
		if ($Status=='Unassigned') {
			...
			$query="SELECT * FROM cyrusbackend.vallcomplaintsd WHERE ComplaintID=$ComplaintID";
			$result=mysqli_query($con,$query);
			$row = mysqli_fetch_array($result);
			...
			$query3="SELECT * FROM cyrusbackend.bank WHERE BankName='$BANK'";
			$result3=mysqli_query($con,$query3);
			$row3 = mysqli_fetch_array($result3);
			$BANKINI=$row3['BankInitial'];
			

			$query2="SELECT * FROM cyrusbackend.employees WHERE EmployeeCode=$EmployeeID";
			$result2=mysqli_query($con,$query2);
			$row2 = mysqli_fetch_array($result2);
			$Phone=$row2['Phone'];
			...
			$sql2 = "INSERT INTO cyrusbackend.sms (Type, ID, EmployeeID, Sent, AssignDate, AssignedBy, AssignType, `Number`, Message)
			...
			$sql = "UPDATE complaints SET EmployeeCode= $EmployeeID, AssignDate='$Date' WHERE ComplaintID=$ComplaintID";
			$sql2 = "INSERT INTO cyrusbackend.sms (Type, ID, EmployeeID, Sent, AssignDate, AssignedBy, AssignType, `Number`, Message)
			VALUES ('c', '$ComplaintID', '$EmployeeID', '0', '$Date', '$User', 'R', '$Phone', '$Message')";
