...
{   
    ...
    if (mysqli_num_rows($resultsCard)>0)
    {
        ...
        while ($row=mysqli_fetch_assoc($resultsCard))
        {
            ...
            if ($Type=='Dataentry' and !empty($row["ServiceDone"])) {
             $ServiceDone='<a data-bs-toggle="modal" data-bs-target="#EditServiceDone" style="color:blue" class="EditServiceDone" id="'.$row["ServiceDone"].'" id2="'.$row["Card Number"].'">'.$row["ServiceDone"].'</a>';
         }else{
            $ServiceDone=$row["ServiceDone"];
        }

        if ($Type=='Dataentry' and !empty($row["WorkPending"])) {
         $PendingWork='<a data-bs-toggle="modal" data-bs-target="#EditPendingWork" style="color:blue" class="EditPendingWork" id="'.$row["WorkPending"].'" id2="'.$row["Card Number"].'">'.$row["WorkPending"].'</a>';
     }else{
        $PendingWork='<a data-bs-toggle="modal" data-bs-target="#EditPendingWork" style="color:blue" class="EditPendingWork" id="Enter Work" id2="'.$row["Card Number"].'">No Pending Work</a>';
    }
    ...
    print '<td>';
    /*if ($Type=='Reporting' and !empty($GadgetID)) {
        $querygadget="SELECT * FROM gadget WHERE GadgetID=$GadgetID";
        $resultGadget=mysqli_query($con,$querygadget);
        if (mysqli_num_rows($resultGadget)>0){
            $rowG=mysqli_fetch_assoc($resultGadget);
        ...
        ?>
        <select class="form-control rounded-corner" id="ChangeGadgetB" id2="<?php echo $row["Card Number"]; ?>">
            <option value="<?php echo $GadgetID ?>"><?php echo $Gadget ?></option>
            <?php 
            $querygadget="SELECT * FROM gadget WHERE GadgetID!=$GadgetID";
            $resultGadget=mysqli_query($con,$querygadget);
            while($rowG=mysqli_fetch_assoc($resultGadget)){
                $Gadget=$rowG["Gadget"];
                ?>
                <option value="<?php echo $rowG["GadgetID"] ?>"><?php echo $rowG["Gadget"] ?></option>
            <?php } ?>
        </select>
        ...
        if ($Type=='Dataentry' and !empty($GadgetID)) {

        ?>
        <a href="" class="ViewGadgetDetails" data-bs-toggle="modal" data-bs-target="#ViewGadgetDetails" id="<?php echo $row["Card Number"]; ?>"><?php echo $Gadget; ?></a>
        <?php 
    }else{
        print $Gadget;
    ...
    print "</td>";
    print '<td>'.$Employee."</td>";
    print '<td style="min-width: 800px;">'.$ServiceDone."</td>";
    print '<td style="min-width: 800px;">'.$PendingWork."</td>"; 
    print '</tr>';
} 
