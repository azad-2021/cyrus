...
{   
  ...
  //echo $EmployeeID;
  $query="SELECT * FROM cyrusbackend.vallordersd WHERE AssignDate is null and Attended=0 and Discription like '%AMC%' and vallordersd.EmployeeCode=$exEmployeeID";
    ...
    <table class="container table table-hover table-bordered border-primary table-responsive">
      ...
      <tbody>
        ...
        if (mysqli_num_rows($result)>0)
        {
          ...
          WHERE AssignDate is null and Attended=0 and Discription like '%AMC%' and vallordersd.EmployeeCode=$exEmployeeID and districts.SubControllerID=$EXEID";
  ...
  <div class="col-lg-12" style="margin: 12px;">
    <table class="container table table-hover table-bordered border-primary table-responsive">
      ...
      <tbody>
        ...
        join `cyrus regions` on districts.RegionCode=`cyrus regions`.RegionCode
        WHERE districts.SubControllerID=$EXEID and vallordersd.AssignDate is null and Attended=0 and vallordersd.Discription like '%AMC%' and BankName='$Bank' and ZoneRegionName='$Zone' group by OrderID order by OrderID";
        $result2=mysqli_query($con,$query2);
        while($row = mysqli_fetch_array($result2)){
