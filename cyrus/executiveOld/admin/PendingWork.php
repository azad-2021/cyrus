        ...
        <div class="table-responsive container">
          ...
          <tbody >
            ...
            while($row=mysqli_fetch_assoc($result)){
              ...
              join cyrusbackend.districts on branchdetails.Address3=districts.District
              join cyrusbackend.`cyrus regions` on districts.RegionCode=`cyrus regions`.RegionCode
              WHERE BankCode=$BankCode and ZoneRegionCode=$ZoneCode  and AssignDate is not null and Attended=0 and Discription not like '%AMC%' and ControlerID=$EXEID";
              ...
              join cyrusbackend.districts on branchdetails.Address3=districts.District
              join cyrusbackend.`cyrus regions` on districts.RegionCode=`cyrus regions`.RegionCode
              WHERE BankCode=$BankCode and ZoneRegionCode=$ZoneCode  and AssignDate is not null and Attended=0 and Discription like '%AMC%' and ControlerID=$EXEID";
              if (mysqli_num_rows($result2)>0){
                $result2=mysqli_query($con,$query);
                $row2=mysqli_fetch_assoc($result2);
              ...
              join cyrusbackend.districts on branchdetails.Address3=districts.District
              join cyrusbackend.`cyrus regions` on districts.RegionCode=`cyrus regions`.RegionCode
              WHERE BankCode=$BankCode and ZoneRegionCode=$ZoneCode  and AssignDate is not null and Attended=0 and ControlerID=$EXEID" ;
              if (mysqli_num_rows($result2)>0){
                $result2=mysqli_query($con,$query);       
                $row2=mysqli_fetch_assoc($result2);
