  ...
  <table class="table table-hover table-bordered border-primary display" id="example3" width="100%">
    ...
    <tbody >
      <?php 
        //echo $EXEID;
      if ($Type=="Executive") {
        $query="SELECT BankName, ZoneRegionName, BranchName, Address3, EmployeeCode, `Employee Name`, OrderID, DateOfInformation, AssignDate, datediff(DATE_ADD(AssignDate,INTERVAL 60 day), current_date()) As LeftDays, Discription FROM allorders
        join cyrusbackend.districts on allorders.Address3=districts.District
        join cyrusbackend.`cyrus regions` on districts.RegionCode=`cyrus regions`.RegionCode
