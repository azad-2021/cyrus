...
$query="SELECT max(VisitDate) FROM cyrusbackend.`jobcard reminder`
inner join cyrusbackend.jobcardmain on `jobcard reminder`.`Card Number`=jobcardmain.`Card Number`";
$result=mysqli_query($con,$query);
$row = mysqli_fetch_array($result);
$SDate=$row["max(VisitDate)"];
          ...
          <center>
            ...
            <div class="table-responsive container">
              <table width="100%" class="table display text-start align-middle table-bordered border-primary table-hover mb-0">
                ...
                <tbody >
                  <?php 
                  $query="SELECT DISTINCT BranchCode FROM cyrusbackend.jobcardmain WHERE length(WorkPending)>2 and VisitDate between '$SDate' and '$EDate'";
                  ...
                  while($row = mysqli_fetch_array($result)){
                    ...
                    while($rowG = mysqli_fetch_array($resultG)){
                      ...
                      $query1="SELECT BankName, ZoneRegionName, BranchName, `Card Number`, VisitDate, WorkPending FROM cyrusbackend.jobcardmain 
                      join branchdetails on jobcardmain.BranchCode=branchdetails.BranchCode
                      WHERE NOT exists (SELECT * FROM cyrusbackend.`jobcard reminder` WHERE `jobcard reminder`.`Card Number`=jobcardmain.`Card Number`) and length(WorkPending)>10 and VisitDate between '$SDate' and '$EDate' and jobcardmain.BranchCode=$BranchCode and GadgetID=$GadgetID order by Job_Card_No desc limit 1";
                      ...
                      while($row1 = mysqli_fetch_array($result1)){
                        ...
                        <tr>
                          ...
                          <td><span class="d-none"><?php echo $row1["VisitDate"]; ?></span><?php echo  date('d-M-Y',strtotime($row1["VisitDate"])); ?></td>
