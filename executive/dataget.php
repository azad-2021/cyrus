...
$SaaSRefID=!empty($_POST['ReferenceID'])?$_POST['ReferenceID']:'';

if (!empty($SaaSRefID))
{   

  $SaaSRefID=!empty($_POST['ReferenceID'])?$_POST['ReferenceID']:'';
  $GadgetID=!empty($_POST['GadgetID'])?$_POST['GadgetID']:'';
  $SIMProvider=!empty($_POST['SIMProvider'])?$_POST['SIMProvider']:'';
  $SIMType=!empty($_POST['SIMType'])?$_POST['SIMType']:'';
  $Operator=!empty($_POST['Operator'])?$_POST['Operator']:'';
  $Validity=!empty($_POST['Validity'])?$_POST['Validity']:0;
  $Remark=!empty($_POST['Remark'])?$_POST['Remark']:'';

  $Zone1=!empty($_POST['Zone1'])?$_POST['Zone1']:'';
  $Zone2=!empty($_POST['Zone2'])?$_POST['Zone2']:'';
  $Zone3=!empty($_POST['Zone3'])?$_POST['Zone3']:'';
  $Zone4=!empty($_POST['Zone4'])?$_POST['Zone4']:'';


  $query="SELECT orders.BranchCode FROM cyrusbackend.branchdetails
  join cyrusbackend.orders on branchdetails.BranchCode=orders.BranchCode
  WHERE orders.OrderID=$SaaSRefID";
  $result=mysqli_query($con,$query);
  if (mysqli_num_rows($result)>0)
  {
  ...
  if($GadgetID==5) {
    ...
    $Category='Zone 1 = '.$Zone1.';
    Zone 2 = '.$Zone2;

    if (!empty($Operator)) {

     $sql="INSERT INTO `orders`(`BranchCode`, `GadgetID`, `SimProvider`, `SimType`, `OperatorID`, `ValidityRecharge`, `Executive`, `VoiceMessage`, `Remark`, `RefID`) VALUES ($BranchCode, $GadgetID, '$SIMProvider', '$SIMType', $Operator, $Validity, '$username', '$Category', '$Remark', $SaaSRefID)";
   }else{

    $sql="INSERT INTO `orders`(`BranchCode`, `GadgetID`, `SimProvider`, `SimType`, `ValidityRecharge`, `Executive`, `VoiceMessage`, `Remark`, `RefID`) VALUES ($BranchCode, $GadgetID, '$SIMProvider', '$SIMType', $Validity, '$username', '$Category', '$Remark', $SaaSRefID)";
  }
...
if ($con4->query($sql) === TRUE) {
  ...
  echo "Error: " . $sql . "<br>" . $con4->error;
}


}

$CheckForSaaS=!empty($_POST['CheckForSaaS'])?$_POST['CheckForSaaS']:'';
if (!empty($CheckForSaaS))
{
  $query="SELECT * FROM cyrusbilling.add_product
  inner join cyrusbackend.item on add_product.IttemID=item.ItemID
  WHERE item.ItemName like '%Sirius%' and order_id=$CheckForSaaS;";
  $result = mysqli_query($con,$query);
  if(mysqli_num_rows($result)>0)
  {
    echo 1;
