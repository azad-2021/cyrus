          ...
          <div class="table-responsive container">
            <table width="100%" class="table display text-start align-middle table-bordered border-primary table-hover mb-0">
              ...
              <tbody>
                ...
                WHERE Inservice=1 Order By `Employee Name`";
                ...
                while($rowE=mysqli_fetch_assoc($resultTech)){
                 ...
                 $query="SELECT count(DISTINCT ComplaintID) FROM cyrusbackend.allcomplaint WHERE AssignDate is not null and Attended=0 and EmployeeCode=$EmployeeID";
                 $result=mysqli_query($con,$query);
                 
                 if(mysqli_num_rows($result)>0)
                 {
                  $row = mysqli_fetch_array($result);
                  $Complaint=$row["count(DISTINCT ComplaintID)"];
                }
                $query2="SELECT count(OrderID) FROM cyrusbackend.allorders WHERE AssignDate is not null and Attended=0 and Discription like '%AMC%' and EmployeeCode=$EmployeeID";
                ...
                $query3 = "SELECT count(OrderID) FROM cyrusbackend.allorders
                WHERE EmployeeCode=$EmployeeID and AssignDate is not NULL and Attended=0 and Discription not like '%AMC%'";
                ...
                if ($row["count(DISTINCT ComplaintID)"]>0 or $AMC>0 or $AO>0) {             
                 ...
                 <tr>
                  ...
                  <td><a class="view_AC" id="<?php print $EmployeeID; ?>" data-bs-target="#ViewAC"><?php echo  $Complaint; ?></a></td>
      ...
      <div class="card-body">
        ...
        <div class="table-responsive container">
          ...
          <tbody >
            ...
            $query="SELECT BankName, zoneregionname, ZoneRegionCode as ZoneCode, BankCode FROM cyrusbackend.branchdetails group by BankCode, ZoneregionCode Order By BankName";
            ...
            while($row=mysqli_fetch_assoc($result)){
              ...
              WHERE BankCode=$BankCode and ZoneRegionCode=$ZoneCode  and AssignDate is not null and Attended=0 and Discription not like '%AMC%'";
              ...
              $query="SELECT count(OrderID) FROM cyrusbackend.orders 
              join branchdetails on orders.BranchCode=branchdetails.BranchCode
              WHERE BankCode=$BankCode and ZoneRegionCode=$ZoneCode  and AssignDate is not null and Attended=0 and Discription like '%AMC%'";
              if (mysqli_num_rows($result2)>0){
                $result2=mysqli_query($con,$query);
                $row2=mysqli_fetch_assoc($result2);
              ...
              WHERE BankCode=$BankCode and ZoneRegionCode=$ZoneCode  and AssignDate is not null and Attended=0" ;
              if (mysqli_num_rows($result2)>0){
                $result2=mysqli_query($con,$query);       
                $row2=mysqli_fetch_assoc($result2);
