...
if (!empty($Data)){
  ...
  <div class="col-lg-12" style="margin: 12px;">
   <table class="container table table-hover table-bordered border-primary table-responsive">
    ...
    <tbody>
     <?php 
     $query2="SELECT DISTINCT demandextended.ItemID, ItemName, ItemQty FROM cyrusbackend.demandextended join cyrusbackend.item on demandextended.ItemID=item.ItemID WHERE OrderID=$OrderID and demandextended.ItemQty !=0";
     ...
     if (mysqli_num_rows($result2)>0)
     {
      ...
      while($row = mysqli_fetch_array($result2)){
        ...
        <tr>
          ...
          <td>
            <select id="alt" class="form-control my-select3">
              <?php 
