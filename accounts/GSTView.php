    ...
    <tbody>
      ...
      if (mysqli_num_rows($result)>0)
      {
       while($row = mysqli_fetch_array($result)){
        ...
        print '<td style="color:blue" class="PaymentUpdate" data-bs-toggle="modal" data-bs-target="#GSTPayment" BillDate="'.$BillDate.'" SGST="'.$SGST.'" CGST="'.$CGST.'" IGST="'.$IGST.'"  Billno="'.$BillNo.'" Totalamount="'.$BillAmount.'" Receiveamount="'.$ReceiveAmount.'" DD="'.$row['DD/Online'].'" SAmount="'.$row['SecurityAmt'].'" SDt="'.date("d-M-Y", strtotime($row['SecurityDt'])).'" SRAmount="'.$row['SecurityRcdAmt'].'" SRDt="'.$row['SecurityRcdDt'].'" Remark="'.$row['Remark'].'" ReceiveDate="'.$ReceiveDate.'" " >&#x20B9 '.$BillAmount."</td>"; 
